...
public class ProjectListOld {
	...
	/*@Test(priority = 4)
	public static void sort_by_language() throws Exception
	{
		Thread.sleep(2000);
		GammaLogin.driver.findElement(By.cssSelector("div.dropdown_container div.dropdown_arrow")).click();

		Thread.sleep(2000);
		GammaLogin.driver.findElement(By.cssSelector(".dropdown_list_data:nth-child(3)")).click();
		Thread.sleep(2000);

		GammaLogin.driver.findElement(By.cssSelector(".button_small:nth-child(1)")).click();
		Thread.sleep(2000);
		Log.info("Project List after Sorting by Rating :- ");
		for (int i = 1; i <= Project.project_count; i++)
		{
			List<WebElement> project_details = GammaLogin.driver.findElements(By.cssSelector("div.project_list:nth-child("+i+")"));
				for (WebElement we1 : project_details) 
				{
					project_rating = GammaLogin.driver.findElement(By.cssSelector("div.project_languages:nth-child("+i+") div.project_rating div.h2"))
							.getText();
					Log.info(project_rating);
				}
			}
			GammaLogin.close_browser();
		}
	*/
	...
	public static void sort_by_size() throws Exception
	{
		...
		/*for (int i = 1; i <= Project.project_count; i++)
		{
			List<WebElement> project_details = GammaLogin.driver.findElements(By.cssSelector("div.project_list:nth-child("+i+")"));
				for (WebElement we1 : project_details) 
				{
					LOC_count = GammaLogin.driver.findElement(By.cssSelector("div.project_list:nth-child("+i+") div.loc_count")).getText();
					LOC_count = LOC_count.substring(6, 11);
					Log.info("LOC Count :- "+LOC_count);
				}
			}
			GammaLogin.close_browser();
		}*/
	...
	/*
	 * @Test(priority = 4) public static void switch_to_listView() throws
	 * Exception {
	 ...
	 * }
	 * 
	 * @Test(priority = 5) public static void switch_to_gridView() throws
	 * Exception {
	 * wait.until(ExpectedConditions.elementToBeClickable(By.cssSelector(
	 * "#Grid_View")));
