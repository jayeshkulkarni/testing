...
public class SubSystemOverviewPanelNew 
{
	...
	public static String[] myStringArray = new String[]
		{"Subsystem Overview","Overall Rating", "Hotspot Distribution","Hotspots",
		 "Heatmap", "Largest 30 Components", "Duplication Distribution","Duplication Percentage"};
  ...
  public static void navigateSubSystemOverviewPanel() 
  {
	  ...
	  we.click();
		 ...
		 List<WebElement>we1 = wdWait.until(ExpectedConditions.presenceOfAllElementsLocatedBy(By.cssSelector(".dashboard_plugin_name")));
		 ...
		 Log.info("Following Panels are present :- ");
		  Log.info("");
		  List<WebElement> list_web_ele2 = driver.findElements(By.cssSelector("div.issue_name"));
		  int i=0;
		  
		  for(WebElement we2:list_web_ele2)
		  {
			  Log.info(we2.getText());
			  Assert.assertEquals(we2.getText(),myStringArray[i]);
			  i++;
		  }
		 
		 
		 we = wdWait.until(ExpectedConditions.presenceOfElementLocated(By.cssSelector("li.plugin_details_wrapper:nth-child(1)")));
		  we.click();
		  Log.info("Page Title is :- "+GammaLogin.driver.getTitle());
		
		  //we = wdWait.until(ExpectedConditions.presenceOfElementLocated(By.cssSelector(".plugin_details_wrapper:nth-child(1)")));
		 // we.click();
		  panelTitleExpect = driver.findElement(By.cssSelector(".panel_title")).getText();
